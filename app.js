...
app.get("/states/:stateId", authenticateToken, async (request, response) => {
  const getStatesQuery = `
    ...
    `;
  const statesArray = await database.all(getStatesQuery);
  response.send(convertStateObjToResponseObj(eachState));
});
...
app.get(
  ...
  async (request, response) => {
    ...
    `;
    const statesArray = await database.all(getDistrictsQuery);
    response.send(convertDistrictDbObjectToResponseObject(eachState));
  }
);
